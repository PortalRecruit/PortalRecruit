...
def _render_profile_overlay(player_id: str):
    ...
    if hasattr(st, "dialog"):
        with st.dialog("Player Profile"):
            if st.button("â Close", key="close_profile_top"):
                _clear_qp("player")
                st.rerun()
            body()
    else:
        st.markdown("---")
        if st.button("â Close Profile", key="close_profile"):
            _clear_qp("player")
            st.rerun()
        body()
...
if st.session_state.app_mode == "Admin":
    ...
    render_header()
    st.caption("âï¸ Ingestion Pipeline & Settings")
    ...
    if query:
        ...
        for hit, phrase in intents.values():
            ...
            explain.append(f"Matched '{phrase}' â {list(intent.traits.keys())}")
        ...
        if not play_ids:
            ...
            for pid, desc, gid, clock, player_id, player_name in play_rows:
                ...
                reason = " â ".join(reason_parts) if reason_parts else "solid all-around fit"

                # Similar NCAA + NBA comps
                sim_players = []
                nba_floor = nba_ceiling = "â"
                try:
                    # Build player vector
                    pstats = player_stats.get(player_id, {})
                ...
                rows.append({
                    ...
                    "Why": reason,
                    "Strengths": ", ".join(strengths) if strengths else "â",
                    "Weaknesses": ", ".join(weaknesses) if weaknesses else "â",
                    "Similar NCAA": ", ".join([s for s in sim_players if s]) if sim_players else "â",
                    "NBA Floor": nba_floor or "â",
                    "NBA Ceiling": nba_ceiling or "â",
